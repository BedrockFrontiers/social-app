...
export default async function UserBox({ me, user, bigger = false, full = false }) {
	...
	return (
		<Link href={`/profile/${user.identifier}`} className="flex gap-2">
		  <div className="w-[50px] h-[50px] flex-shrink-0 rounded-full">
		    <Image className="rounded-full object-cover select-none h-[50px]" src={user.avatarUrl} width={size[0]} height={size[0]} quality={100} alt="Profile Picture" />
		  </div>
		  <div>
		    ...
		    <p className={`text-${size[3]} text-zinc-500 max-w-[130px] truncate font-semibold`}>{user.identifier}</p>
		    {full && (
