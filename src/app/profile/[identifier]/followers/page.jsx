...
export default async function ProfileFollowers({ params }) {
	...
	return (
		<MainStructure>
			<MainHeader returnRoute={`/profile/${user.identifier}`}>
				<h3 className="select-none font-bold text-xl">{ user.identifier }&#39;s followers</h3>
			</MainHeader>
			<div className="mt-5">
				{user.followers.map((follower, index) => (
					<div key={index} className="themed-border !border-x-0 !border-t-0">
						<div className="px-4 pb-4">
							<UserBox bigger={true} full={true} me={me} user={follower.following} />
						</div>
					</div>
				))}
			</div>
